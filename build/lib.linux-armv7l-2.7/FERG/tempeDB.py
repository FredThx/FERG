...
class conso_electriqueDB(DB_buffered):
	...
	def archive(self, date_max):
		'''Archive (à l'heure) les conso electriques
			puissance : moyenne(puissance)
			energie : somme(energie)
			compteur : dernier(compteur)
		'''
		cursor = self.db.cursor()
		req = "SELECT Id, circuit, date_debut, date_fin, energie, puissance, compteur, type_horaire"
		#TODO.......
